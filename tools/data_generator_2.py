...
image_folder = 'data/imagenet/train'
...
#         length = min(batch_size, (len(self.names) - i))
        length = 1
        batch_x = np.empty((1,img_rows, img_cols), dtype=np.float32)
        batch_y = np.empty((self.nb_q, out_img_rows, out_img_cols), dtype=np.float32)
        ...
        while(cnt <length):
            ...
            name_part[1] = 'JPEG'
            name_fix = ''.join([name_part[0],'.',name_part[1]])
            
            filename = os.path.join(image_folder, dir_name, name_fix)
            ...
            if type(gray) is np.ndarray:        
                ...
                batch_x[0,:,:] = x.copy()
                batch_y = y.transpose([2,0,1]).copy()
#                 batch_x= x.copy()
#                 batch_y= y.copy()
